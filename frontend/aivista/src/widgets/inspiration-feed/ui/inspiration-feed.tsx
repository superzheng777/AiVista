  ...
  return (
    <main className="min-h-dvh bg-[var(--page-bg)] px-4 py-7 text-[var(--primary)] sm:px-8 sm:py-9 lg:px-12 lg:py-[34px]">
      <div className="mx-auto w-full max-w-[1640px]">
        <header className="relative min-h-[126px]">
          <p className="text-[11px] font-semibold leading-none tracking-[0.16em] text-[var(--text-secondary)]">
            <span className="text-[var(--accent)]">01</span> / EXPLORE
          </p>
          <h1 className="mt-[14px] text-[32px] font-bold leading-[1.2] tracking-tight sm:text-4xl">灵感探索</h1>
        ...
        <nav aria-label="作品列表" className="flex h-[62px] items-end justify-between border-b border-[var(--border)]">
          <FeedTab href="/inspirations" active={!following}>
            发现
          </FeedTab>
          <FeedTab href="/inspirations?view=following" active={following}>
            关注
          </FeedTab>
          <InspirationSearchForm compact />
        </nav>

        <section className="mb-3 mt-[18px] flex items-center">
          <h2 className="shrink-0 text-lg font-bold">{following ? "关注作品" : "今日灵感"}</h2>
          <p className="ml-5 shrink-0 text-[13px] text-[var(--text-secondary)] sm:ml-[30px]">
            {following ? "来自你关注的创作者" : "持续发现新的视觉表达"}
          </p>
          <span className="ml-4 h-px flex-1 bg-[var(--border)] sm:ml-[18px]" />
          <AccentSquare size={8} className="ml-4 shrink-0" />
        </section>
        ...
        {enabled && inspirations.isError ? (
          <section role="alert" className="border border-[var(--accent-border)] bg-[var(--surface-bg)] p-8 text-center">
            <p className="text-sm text-[var(--accent-hover)]">
              {following ? "关注列表加载失败。" : "发现列表加载失败。"}
            </p>
            <button
              type="button"
              onClick={() => void inspirations.refetch()}
        ...
        {enabled && inspirations.isSuccess && images.length === 0 ? (
          <EmptyState
            message={
              following && selfProfile.isLoading
                ? "正在确认关注状态…"
                : following
        ...
        {enabled ? (
          <div ref={loadMoreRef} className="min-h-12" aria-live="polite">
            {isFetchingNextPage ? (
              <p className="pt-4 text-center text-sm text-[var(--text-secondary)]">正在加载更多</p>
            ) : null}
...
function FeedTab({ href, active, children }: { href: string; active: boolean; children: string }) {
  return (
    <Link
      ...
      className={cn(
        "relative mr-10 flex h-full items-center text-[17px] transition-colors focus-visible:outline-2 focus-visible:outline-offset-2",
        active ? "font-semibold text-[var(--primary)]" : "text-[var(--text-secondary)] hover:text-[var(--primary)]",
      )}
    >
      {children}
...
function FeedSkeleton() {
  return (
    <div className="columns-2 gap-[18px] sm:columns-3 lg:columns-4 min-[1600px]:columns-5">
      {Array.from({ length: 10 }, (_, index) => (
        <div
          key={index}
          className="mb-[18px] break-inside-avoid animate-pulse overflow-hidden rounded-[8px] border border-[var(--border)] bg-[var(--surface-bg)]"
        >
          <div className={cn("bg-[var(--skeleton)]", index % 3 === 0 ? "h-72" : index % 3 === 1 ? "h-48" : "h-60")} />
          <div className="h-[52px] border-t border-[var(--border)]" />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <section className="mx-auto mt-24 max-w-[420px] text-center">
      <div aria-hidden="true" className="relative mx-auto h-14 w-16">
        ...
        <AccentSquare size={8} className="absolute bottom-1 left-0" />
        <span className="absolute right-0 top-0 text-lg leading-none">✦</span>
      </div>
      <p className="mt-5 text-sm text-[var(--text-secondary)]">{message}</p>
      {action && href ? (
        <Link href={href} className="mt-4 inline-flex text-sm font-medium text-[var(--accent)] hover:underline">
          {action}
        </Link>
      ) : null}
      {action && onAction ? (
