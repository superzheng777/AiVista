...
export function ImageDetailShell({ image, onClose, actions, author, allowCopy = false, timeLabel = "生成时间", timeValue = image.createdAt, refreshImage }: ImageDetailShellProps) {
  ...
  async function download(): Promise<void> {
    ...
    if (!user || user.id !== image.authorId) return;
  ...
  return <section className="grid min-h-screen grid-cols-[minmax(0,1fr)_380px] bg-muted/35">
    ...
    <aside className="min-h-0 overflow-y-auto border-l border-border bg-card">
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-card/95 px-5 py-4 backdrop-blur"><button type="button" onClick={onClose} className="grid size-9 place-items-center rounded-lg text-muted-foreground hover:bg-muted" aria-label="关闭详情"><X className="size-5" /></button><div className="flex gap-1">{status !== "authenticated" || user?.id === image.authorId ? <button type="button" onClick={() => void download()} className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-border px-3 text-sm font-medium hover:bg-muted"><Download className="size-4" />下载</button> : null}{allowCopy ? <button type="button" onClick={() => void copy()} className="grid size-9 place-items-center rounded-lg text-muted-foreground hover:bg-muted" aria-label="复制图片"><Clipboard className="size-4" /></button> : null}</div></div>
