...
export function PublicUserPage({ userId }: { userId: string }) {
  ...
  return <main className="mx-auto max-w-6xl px-6 py-9">
    ...
    <Section title="作品" loading={works.isLoading} images={works.data ?? []} onOpen={setSelected} />
    {(isSelf || author.likesPublic) ? <Section title="点赞" loading={likes.isLoading} images={likes.data ?? []} onOpen={setSelected} /> : null}
...
function Section({ title, images, loading, onOpen }: { title: string; images: GenerationAsset[]; loading: boolean; onOpen: (image: GenerationAsset) => void }) {
  return <section className="mt-8"><h2 className="text-lg font-semibold">{title}</h2>{loading ? <p className="mt-3 text-sm text-muted-foreground">加载中…</p> : !images.length ? <p className="mt-3 text-sm text-muted-foreground">暂无{title}。</p> : <div className="mt-4 grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-4">{images.map((image) => <button key={image.id} type="button" onClick={() => onOpen(image)} className="overflow-hidden rounded-2xl border bg-card text-left hover:shadow-md"><img src={image.url} alt={image.title ?? title} className="aspect-square w-full object-cover" /><div className="flex justify-between gap-2 p-3 text-sm"><span className="truncate">{image.title ?? "未命名作品"}</span><span className="inline-flex shrink-0 items-center gap-1 text-xs text-muted-foreground"><Heart className="size-3" />{image.likeCount}</span></div></button>)}</div>}</section>;
}
