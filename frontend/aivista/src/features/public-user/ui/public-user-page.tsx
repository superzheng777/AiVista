...
export function PublicUserPage({ userId }: { userId: string }) {
  ...
  return <main className="min-h-screen bg-[var(--page-bg)] px-4 py-5 text-[var(--primary)] sm:px-8 sm:py-8 lg:px-12">
    <div className="mx-auto max-w-[1680px] space-y-5">
      ...
      <section className="flex min-h-16 flex-wrap items-center justify-between gap-3 rounded-[10px] border border-[var(--border)] bg-[var(--surface-bg)] px-5 shadow-[0_1px_2px_rgb(43_35_25_/_4%)] sm:px-10">
        ...
        <button type="button" onClick={startCreating} className="inline-flex h-[42px] items-center gap-2 rounded-[7px] bg-[var(--accent)] px-5 text-sm font-semibold text-[var(--surface-bg)] transition-colors hover:bg-[var(--accent-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2"><Sparkles className="size-4" />开始创作</button>
      </section>

      {notice ? <div role="status" className="flex items-center justify-between rounded-[7px] border border-[var(--border)] bg-[var(--surface-bg)] px-4 py-3 text-sm text-[var(--text-secondary)]"><span>{notice}</span><button type="button" onClick={() => setNotice(null)} className="rounded p-1 text-[var(--text-secondary)] hover:bg-[var(--page-bg)]" aria-label="关闭提示"><X className="size-4" /></button></div> : null}
...
function ProfileHero({ author, isSelf, editing, nickname, bio, saveError, isSaving, isFollowing, followLabel, likesPublic, isUpdatingVisibility, onFollow, onStartEditing, onCancelEditing, onNicknameChange, onBioChange, onSave, onLogout, onVisibilityChange }: ProfileHeroProps) {
  return <section className="relative min-h-[320px] overflow-hidden rounded-[12px] border border-[var(--border)] bg-[var(--surface-bg)] px-6 py-8 shadow-[0_1px_2px_rgb(43_35_25_/_4%),0_8px_24px_rgb(43_35_25_/_5%)] sm:px-12 sm:py-[42px]">
    <HeroDecorations />
    <div className="relative z-10 flex min-h-[254px] flex-col">
      <p className="text-[11px] font-semibold tracking-[0.16em] text-[var(--text-secondary)]"><span className="text-[var(--accent)]">01</span> / CREATOR PROFILE</p>
      <div className="mt-5 flex flex-1 flex-col justify-between gap-6 lg:flex-row lg:items-end">
        <div className="flex min-w-0 items-center gap-5 sm:gap-7">
          <Avatar author={author} />
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2"><h1 className="truncate text-3xl font-bold leading-[1.15] tracking-tight text-[var(--primary)] sm:text-[36px]">{author.nickname}</h1><span className="inline-flex h-[30px] items-center rounded-[6px] border border-[var(--accent-border-strong)] bg-[var(--accent-soft)] px-2.5 text-sm font-medium text-[var(--accent)]"><Sparkles className="mr-1 size-3.5" />创作者</span></div>
            <p className="mt-3 max-w-xl whitespace-pre-wrap text-sm leading-6 text-[var(--text-secondary)] sm:text-base">{author.bio || "这个作者还没有简介。"}</p>
            <dl className="mt-5 flex flex-wrap items-center gap-y-3 text-sm text-[var(--text-secondary)]"><ProfileStat value={author.followerCount} label="关注者" /><ProfileStat value={author.followingCount} label="正在关注" /><ProfileStat value={author.receivedLikeCount} label="获赞" /></dl>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-3 self-end">
          {isSelf ? <><button type="button" onClick={onStartEditing} className="inline-flex h-[46px] items-center gap-2 rounded-[7px] bg-[var(--primary)] px-5 text-sm font-semibold text-[var(--surface-bg)] transition-colors hover:bg-[var(--primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2"><Pencil className="size-4" />编辑资料</button><button type="button" onClick={onLogout} className="inline-flex h-[46px] items-center gap-2 rounded-[7px] border border-[var(--text-secondary)] bg-[var(--surface-bg)] px-5 text-sm font-semibold text-[var(--primary)] transition-colors hover:bg-[var(--page-bg)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--text-secondary)] focus-visible:ring-offset-2"><LogOut className="size-4" />退出登录</button></> : <button type="button" onClick={onFollow} disabled={isFollowing} className="inline-flex h-[46px] items-center rounded-[7px] bg-[var(--primary)] px-7 text-sm font-semibold text-[var(--surface-bg)] transition-colors hover:bg-[var(--primary-hover)] disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2">{followLabel}</button>}
        </div>
      </div>
      {isSelf ? <label className="mt-4 flex w-fit items-center gap-2 text-xs text-[var(--text-secondary)]"><input aria-label="公开我的点赞列表" type="checkbox" checked={likesPublic} disabled={isUpdatingVisibility} onChange={(event) => onVisibilityChange(event.target.checked)} className="size-4 accent-[var(--accent)]" />公开我的点赞列表</label> : null}
      {isSelf && editing ? <form className="mt-4 max-w-xl space-y-3 border-t border-[var(--border)] pt-4" onSubmit={onSave}><input aria-label="昵称" value={nickname} onChange={(event) => onNicknameChange(event.target.value)} required minLength={1} maxLength={32} className="h-10 w-full rounded-[7px] border border-[var(--border-strong)] bg-[var(--surface-bg)] px-3 text-sm outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)]/20" /><textarea aria-label="个人简介" value={bio} onChange={(event) => onBioChange(event.target.value)} maxLength={500} rows={3} className="w-full rounded-[7px] border border-[var(--border-strong)] bg-[var(--surface-bg)] px-3 py-2 text-sm outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)]/20" />{saveError ? <p className="text-sm text-[var(--accent)]">{saveError}</p> : null}<div className="flex justify-end gap-2"><button type="button" onClick={onCancelEditing} disabled={isSaving} className="h-9 rounded-[7px] border border-[var(--text-secondary)] px-3 text-sm text-[var(--primary)]">取消</button><button type="submit" disabled={isSaving} className="inline-flex h-9 items-center gap-1 rounded-[7px] bg-[var(--primary)] px-3 text-sm text-[var(--surface-bg)] disabled:opacity-60"><Check className="size-3.5" />保存</button></div></form> : null}
    </div>
  </section>;
}

function HeroDecorations() {
  return <><div aria-hidden className="absolute right-0 top-0 h-[58%] w-[42%] bg-[var(--active-bg)]" /><div aria-hidden className="absolute bottom-0 right-0 h-[42%] w-[35%] bg-[var(--peach)]/45" /><AccentSquare size={22} className="absolute right-6 top-4" /><DotMatrix columns={5} rows={3} dotSize={3} gap={4} color="var(--accent)" opacity={0.25} className="absolute left-12 top-4" /><div aria-hidden className="absolute inset-x-10 bottom-6 h-px bg-[var(--border-strong)]" /></>;
}
...
function Avatar({ author }: { author: Awaited<ReturnType<typeof getPublicAuthor>> }) {
  ...
  return <div className="grid size-28 shrink-0 place-items-center overflow-hidden rounded-full border border-[var(--avatar-border)] bg-[var(--surface-bg)] sm:size-[164px]">{author.avatarUrl ? <img src={author.avatarUrl} alt={`${author.nickname}的头像`} className="size-full object-cover" /> : <span className="grid size-14 place-items-center rounded-[7px] bg-[var(--accent)] text-2xl font-semibold text-[var(--surface-bg)] sm:size-[62px] sm:text-[28px]">{initial}</span>}</div>;
}

function ProfileStat({ value, label }: { value: number; label: string }) {
  return <div className="flex items-center gap-2 border-l border-[var(--border)] pl-5 first:border-l-0 first:pl-0 sm:pr-3"><dd className="text-lg font-semibold text-[var(--primary)]">{value}</dd><dt>{label}</dt></div>;
}

function ProfileTab({ active, id, onClick, children }: { active: boolean; id: string; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" role="tab" id={id} aria-controls="profile-content-panel" aria-selected={active} onClick={onClick} className={cn("border-b-2 px-1 text-base transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]", active ? "border-[var(--primary)] font-semibold text-[var(--primary)]" : "border-transparent text-[var(--text-secondary)] hover:text-[var(--primary)]")}>{children}</button>;
}
...
function WorkCard({ image, opening, onOpen }: { image: GenerationAsset; opening: boolean; onOpen: (image: GenerationAsset) => void | Promise<void> }) {
  const content = <WorkPreviewCardSurface><ImageThumbnail image={image} /><WorkPreviewCardInfoBar title={image.title ?? "未命名作品"} trailing={image.publicationReviewStatus === "APPROVED" ? <WorkPreviewCardLikes count={image.likeCount} /> : undefined} />{image.publicationReviewStatus === "PENDING" ? <span className="absolute right-3 top-3 rounded-[6px] bg-[var(--accent)] px-2 py-1 text-xs font-medium text-[var(--surface-bg)]">审核中</span> : null}</WorkPreviewCardSurface>;
  const className = "group block w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] disabled:opacity-60";
...
function ImageThumbnail({ image }: { image: GenerationAsset }) {
  return <WorkPreviewCardImage image={image}>{image.imageUrls.thumbnail ? <img src={image.imageUrls.thumbnail.url} alt={image.title ?? "作品"} loading="lazy" decoding="async" referrerPolicy="no-referrer" className="block size-full object-cover" /> : <span className="absolute inset-0 bg-[var(--skeleton-light)]" />}</WorkPreviewCardImage>;
}

function WorkGridSkeleton() {
  return <div className="grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-4">{Array.from({ length: 8 }, (_, index) => <div key={index} className="aspect-square animate-pulse rounded-[10px] bg-[var(--skeleton-dark)]" />)}</div>;
}
...
function ProfileEmptyState({ isSelf, title, description, onStartCreating }: { isSelf: boolean; title: string; description: string; onStartCreating?: () => void }) {
  return <div className="flex min-h-[400px] flex-1 items-start justify-center pt-16 text-center"><div className="max-w-[440px]"><EmptyGeometry /><h2 className="mt-5 text-2xl font-bold leading-[1.25] text-[var(--primary)] sm:text-[28px]">{title}</h2><p className="mt-3 text-[15px] leading-7 text-[var(--text-secondary)]">{description}</p>{isSelf && onStartCreating ? <button type="button" onClick={onStartCreating} className="mt-6 inline-flex h-[46px] min-w-[184px] items-center justify-center gap-2 rounded-[7px] bg-[var(--primary)] px-6 text-sm font-semibold text-[var(--surface-bg)] transition-colors hover:bg-[var(--primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2"><Sparkles className="size-4" />开始创作</button> : null}</div></div>;
}

function EmptyGeometry() {
  return <div aria-hidden className="relative mx-auto h-[86px] w-[124px]"><span className="absolute left-1 top-0 h-[54px] w-[74px] border border-[var(--text-secondary)]" /><span className="absolute left-[39px] top-[18px] h-[54px] w-[74px] border border-[var(--text-secondary)]" /><span className="absolute bottom-0 left-0 size-[10px] bg-[var(--accent)]" /><Sparkles className="absolute right-0 top-[-7px] size-[14px] text-[var(--primary)]" /></div>;
}

function PanelError({ message, onRetry }: { message: string; onRetry: () => void }) {
  return <div role="alert" className="grid min-h-[400px] place-items-center text-center"><div><p className="text-sm text-[var(--accent)]">{message}</p><button type="button" onClick={onRetry} className="mt-3 text-sm font-medium text-[var(--accent)] underline underline-offset-4">重新加载</button></div></div>;
}
