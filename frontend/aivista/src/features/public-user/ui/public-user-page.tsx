...
function Section({ title, images, loading, onOpen }: { title: string; images: GenerationAsset[]; loading: boolean; onOpen: (image: GenerationAsset) => void | Promise<void> }) {
  return <section className="mt-8"><h2 className="text-lg font-semibold">{title}</h2>{loading ? <p className="mt-3 text-sm text-muted-foreground">加载中…</p> : !images.length ? <p className="mt-3 text-sm text-muted-foreground">暂无{title}。</p> : <div className="mt-4 grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-4">{images.map((image) => <Link key={image.id} href={`/inspirations?imageId=${encodeURIComponent(image.id)}`} prefetch={false} onClick={(event) => { if (event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return; event.preventDefault(); void onOpen(image); }} className="overflow-hidden rounded-2xl border bg-card text-left hover:shadow-md">{image.imageUrls.thumbnail ? <img src={image.imageUrls.thumbnail.url} alt={image.title ?? title} loading="lazy" decoding="async" className="aspect-square w-full object-cover" /> : <div className="aspect-square bg-muted" />}<div className="flex justify-between gap-2 p-3 text-sm"><span className="truncate">{image.title ?? "未命名作品"}</span><span className="inline-flex shrink-0 items-center gap-1 text-xs text-muted-foreground"><Heart className="size-3" />{image.likeCount}</span></div></Link>)}</div>}</section>;
}
