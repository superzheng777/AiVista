...
import {
  getWorkPreviewCardHeight,
  WorkPreviewCardImage,
  WorkPreviewCardInfoBar,
  WorkPreviewCardLikes,
...
export function PublicUserPage({ userId }: { userId: string }) {
  ...
  if (profile.isLoading)
    return (
      <main className="min-h-screen bg-[var(--page-bg)] p-10 text-sm text-[var(--text-secondary)]">
        加载作者资料中…
      </main>
    );
  if (profile.isError || !profile.data)
    return (
      <main className="min-h-screen bg-[var(--page-bg)] p-10 text-sm text-[var(--accent)]">
        作者不存在或暂时不可用。
      </main>
    );

  const author = profile.data;
  const followLabel = author.viewerFollowing
  ...
  return (
    <main className="min-h-screen bg-[var(--page-bg)] px-4 py-5 text-[var(--primary)] sm:px-8 sm:py-8 lg:px-12">
      <div className="mx-auto max-w-[1680px] space-y-5">
        <ProfileHero
          ...
          likesPublic={author.likesPublic}
          isUpdatingVisibility={visibility.isPending}
          onFollow={() => {
            if (status === "authenticated") follow.mutate(!author.viewerFollowing);
            else openAuthDialog();
          }}
        ...
        <section className="flex min-h-16 flex-wrap items-center justify-between gap-3 rounded-[10px] border border-[var(--border)] bg-[var(--surface-bg)] px-5 shadow-[0_1px_2px_rgb(43_35_25_/_4%)] sm:px-10">
          <div role="tablist" aria-label="个人主页内容" className="flex h-16 items-stretch gap-10">
            <ProfileTab active={activeView === "works"} id="profile-works-tab" onClick={() => setActiveView("works")}>
              作品
            </ProfileTab>
            {canViewLikes ? (
              <ProfileTab active={activeView === "likes"} id="profile-likes-tab" onClick={() => setActiveView("likes")}>
                点赞
              </ProfileTab>
            ) : null}
          </div>
          <button
            ...
            className="inline-flex h-[42px] items-center gap-2 rounded-[7px] bg-[var(--accent)] px-5 text-sm font-semibold text-[var(--surface-bg)] transition-colors hover:bg-[var(--accent-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2"
          >
        ...
        {notice ? (
          <div
            role="status"
            className="flex items-center justify-between rounded-[7px] border border-[var(--border)] bg-[var(--surface-bg)] px-4 py-3 text-sm text-[var(--text-secondary)]"
          >
            <span>{notice}</span>
            <button
              type="button"
              onClick={() => setNotice(null)}
              className="rounded p-1 text-[var(--text-secondary)] hover:bg-[var(--page-bg)]"
              aria-label="关闭提示"
            >
              <X className="size-4" />
            </button>
        ...
        <section
          ...
          role="tabpanel"
          aria-labelledby={activeView === "works" ? "profile-works-tab" : "profile-likes-tab"}
      ...
      {pendingDetail ? (
        <PendingWorkDetail
          asset={pendingDetail}
          navigation={pendingDetailNavigation}
          isDeleting={removePublicationMutation.isPending}
          onClose={() => setPendingDetailId(null)}
          onDelete={() => setRemoveTarget(pendingDetail)}
          refreshImage={refreshPendingImage}
        />
      ) : null}
      {removeTarget ? (
        <RemovePendingDialog
          ...
          onConfirm={() => removePublicationMutation.mutate(removeTarget.id)}
        />
      ) : null}
      {detail.image ? (
        <PublicImageDetailOverlay
          ...
          navigation={publicDetailNavigation}
        />
      ) : null}
      {detail.openError ? (
        <PublicImageOpenError message={detail.openError} onDismiss={detail.dismissOpenError} />
      ) : null}
    </main>
  );
}
...
function ProfileHero({
  ...
  return (
    <section className="relative min-h-[320px] overflow-hidden rounded-[12px] border border-[var(--border)] bg-[var(--surface-bg)] px-6 py-8 shadow-[0_1px_2px_rgb(43_35_25_/_4%),0_8px_24px_rgb(43_35_25_/_5%)] sm:px-12 sm:py-[42px]">
      <HeroDecorations />
      <div className="relative z-10 flex min-h-[254px] flex-col">
        <p className="text-[11px] font-semibold tracking-[0.16em] text-[var(--text-secondary)]">
          <span className="text-[var(--accent)]">01</span> / CREATOR PROFILE
        </p>
          ...
          <div className="flex min-w-0 items-center gap-5 sm:gap-7">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                <h1 className="truncate text-3xl font-bold leading-[1.15] tracking-tight text-[var(--primary)] sm:text-[36px]">
                  {author.nickname}
                </h1>
                <span className="inline-flex h-[30px] items-center rounded-[6px] border border-[var(--accent-border-strong)] bg-[var(--accent-soft)] px-2.5 text-sm font-medium text-[var(--accent)]">
                  <Sparkles className="mr-1 size-3.5" />
                  创作者
                </span>
              </div>
              <p className="mt-3 max-w-xl whitespace-pre-wrap text-sm leading-6 text-[var(--text-secondary)] sm:text-base">
                {author.bio || "这个作者还没有简介。"}
              </p>
              <dl className="mt-5 flex flex-wrap items-center gap-y-3 text-sm text-[var(--text-secondary)]">
                <ProfileStat value={author.followerCount} label="关注者" />
                <ProfileStat value={author.followingCount} label="正在关注" />
                <ProfileStat value={author.receivedLikeCount} label="获赞" />
              </dl>
            </div>
          </div>
            ...
            {isSelf ? (
              <>
                <button
                  ...
                  className="inline-flex h-[46px] items-center gap-2 rounded-[7px] bg-[var(--primary)] px-5 text-sm font-semibold text-[var(--surface-bg)] transition-colors hover:bg-[var(--primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2"
                >
                ...
                <button
                  type="button"
                  onClick={onLogout}
                  className="inline-flex h-[46px] items-center gap-2 rounded-[7px] border border-[var(--text-secondary)] bg-[var(--surface-bg)] px-5 text-sm font-semibold text-[var(--primary)] transition-colors hover:bg-[var(--page-bg)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--text-secondary)] focus-visible:ring-offset-2"
                >
              ...
              <button
                ...
                className="inline-flex h-[46px] items-center rounded-[7px] bg-[var(--primary)] px-7 text-sm font-semibold text-[var(--surface-bg)] transition-colors hover:bg-[var(--primary-hover)] disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2"
              >
                {followLabel}
              </button>
            )}
        ...
        {isSelf ? (
          <label className="mt-4 flex w-fit items-center gap-2 text-xs text-[var(--text-secondary)]">
            <input
              aria-label="公开我的点赞列表"
              type="checkbox"
              checked={likesPublic}
...
function HeroDecorations() {
  return (
    <>
      <div aria-hidden className="absolute right-0 top-0 h-[58%] w-[42%] bg-[var(--active-bg)]" />
      <div aria-hidden className="absolute bottom-0 right-0 h-[42%] w-[35%] bg-[var(--peach)]/45" />
      <AccentSquare size={22} className="absolute right-6 top-4" />
      <DotMatrix
        columns={5}
        rows={3}
        dotSize={3}
      ...
      />
      <div aria-hidden className="absolute inset-x-10 bottom-6 h-px bg-[var(--border-strong)]" />
    </>
  );
}
...
function Avatar({ author }: { author: Awaited<ReturnType<typeof getPublicAuthor>> }) {
  ...
  return (
    <div className="grid size-28 shrink-0 place-items-center overflow-hidden rounded-full border border-[var(--avatar-border)] bg-[var(--surface-bg)] sm:size-[164px]">
      {author.avatarUrl ? (
        <img src={author.avatarUrl} alt={`${author.nickname}的头像`} className="size-full object-cover" />
      ) : (
        <span className="grid size-14 place-items-center rounded-[7px] bg-[var(--accent)] text-2xl font-semibold text-[var(--surface-bg)] sm:size-[62px] sm:text-[28px]">
          {initial}
        </span>
      )}
    </div>
...
function ProfileStat({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex items-center gap-2 border-l border-[var(--border)] pl-5 first:border-l-0 first:pl-0 sm:pr-3">
      <dd className="text-lg font-semibold text-[var(--primary)]">{value}</dd>
      <dt>{label}</dt>
    </div>
  );
}
...
function ProfileTab({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "border-b-2 px-1 text-base transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]",
        active
...
function WorksPanel({
  ...
  if (!images.length)
    return (
      <ProfileEmptyState
        isSelf={isSelf}
        title={isSelf ? "这里还没有作品" : "这里还没有公开作品"}
        description={isSelf ? "把一次灵感变成你的第一件作品" : "这个创作者还没有公开作品"}
...
function LikesPanel({
  ...
  if (!images.length)
    return <ProfileEmptyState isSelf={false} title="这里还没有点赞作品" description="喜欢的作品会出现在这里" />;
...
function WorkGrid({
  ...
  return (
    <ShortestLaneFeed
      ...
      minLaneWidth={180}
      renderItem={(image) => <WorkCard image={image} opening={openingId === image.id} onOpen={onOpen} />}
    />
  );
}
...
function WorkCard({
  ...
  const content = (
    <WorkPreviewCardSurface>
      ...
      <WorkPreviewCardInfoBar
        title={image.title ?? "未命名作品"}
        trailing={
          image.publicationReviewStatus === "APPROVED" ? <WorkPreviewCardLikes count={image.likeCount} /> : undefined
        }
      />
      {image.publicationReviewStatus === "PENDING" ? (
        <span className="absolute right-3 top-3 rounded-[6px] bg-[var(--accent)] px-2 py-1 text-xs font-medium text-[var(--surface-bg)]">
          审核中
        </span>
      ) : null}
    </WorkPreviewCardSurface>
  ...
  const className =
    "group block w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] disabled:opacity-60";
  if (image.publicationReviewStatus !== "APPROVED")
    return (
      <button type="button" disabled={opening} onClick={() => void onOpen(image)} className={className}>
        {content}
      </button>
    );
  return (
    <Link
      href={`/inspirations?imageId=${encodeURIComponent(image.id)}`}
      prefetch={false}
      onClick={(event) => {
        if (
  ...
  return (
    <WorkPreviewCardImage image={image}>
      {image.imageUrls.thumbnail ? (
        ...
        <span className="absolute inset-0 bg-[var(--skeleton-light)]" />
      )}
    </WorkPreviewCardImage>
  );
}
  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-4">
      {Array.from({ length: 8 }, (_, index) => (
        <div key={index} className="aspect-square animate-pulse rounded-[10px] bg-[var(--skeleton-dark)]" />
      ))}
    </div>
  );
}
...
function ProfileEmptyState({
  ...
  return (
    <div className="flex min-h-[400px] flex-1 items-start justify-center pt-16 text-center">
      <div className="max-w-[440px]">
        <EmptyGeometry />
        <h2 className="mt-5 text-2xl font-bold leading-[1.25] text-[var(--primary)] sm:text-[28px]">{title}</h2>
        <p className="mt-3 text-[15px] leading-7 text-[var(--text-secondary)]">{description}</p>
        {isSelf && onStartCreating ? (
          <button
            type="button"
            onClick={onStartCreating}
            className="mt-6 inline-flex h-[46px] min-w-[184px] items-center justify-center gap-2 rounded-[7px] bg-[var(--primary)] px-6 text-sm font-semibold text-[var(--surface-bg)] transition-colors hover:bg-[var(--primary-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2"
          >
            <Sparkles className="size-4" />
            开始创作
          </button>
...
function EmptyGeometry() {
  return (
    <div aria-hidden className="relative mx-auto h-[86px] w-[124px]">
      <span className="absolute left-1 top-0 h-[54px] w-[74px] border border-[var(--text-secondary)]" />
      <span className="absolute left-[39px] top-[18px] h-[54px] w-[74px] border border-[var(--text-secondary)]" />
      <span className="absolute bottom-0 left-0 size-[10px] bg-[var(--accent)]" />
      <Sparkles className="absolute right-0 top-[-7px] size-[14px] text-[var(--primary)]" />
    </div>
  );
}
...
function PanelError({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div role="alert" className="grid min-h-[400px] place-items-center text-center">
      <div>
...
function ArchiveFooter() {
  return (
    <div aria-hidden className="mt-auto flex items-center gap-3 pb-8 pt-8">
      <span className="size-2 shrink-0 bg-[var(--text-secondary)]" />
      <span className="h-px flex-1 bg-[var(--border-strong)]" />
      <span className="hidden shrink-0 whitespace-nowrap text-[10px] tracking-[0.18em] text-[var(--text-secondary)] sm:block">
  ...
  return (
    <ImageDetailOverlay>
      <ImageDetailShell
        ...
        actions={
          <section>
            <p className="text-xs font-medium tracking-wide text-muted-foreground">发布操作</p>
            <div className="mt-3 rounded-xl bg-muted/60 px-3 py-2.5 text-sm text-muted-foreground">
              作品正在等待审核
            </div>
            <button
              type="button"
              onClick={onDelete}
              disabled={isDeleting}
              className="mt-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-destructive/10 text-sm font-medium text-destructive hover:bg-destructive/20 disabled:opacity-50"
            >
              <X className="size-4" />
              取消审核
            </button>
            <dl className="mt-5 space-y-3 border-t border-border pt-5 text-sm">
              <div>
                ...
                <dd className="mt-1 font-medium">{createdAtText(asset.createdAt)}</dd>
              </div>
            </dl>
          </section>
        }
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Viewport className="fixed inset-0 z-50 grid place-items-center p-4">
          <Dialog.Popup className="w-full max-w-sm rounded-[10px] border border-[var(--border)] bg-[var(--surface-bg)] p-6 shadow-[0_24px_70px_-30px_var(--shadow)]">
            <h2 className="text-lg font-semibold">取消审核？</h2>
            <p className="mt-3 text-sm leading-6 text-muted-foreground">
              作品将从个人主页移除，但原图仍会保留在资产库。
            </p>
            <div className="mt-6 flex justify-end gap-3">
              <button
                ...
                onClick={onCancel}
                disabled={isDeleting}
                className="h-9 rounded-[7px] px-3 text-sm font-medium text-muted-foreground transition hover:bg-[var(--surface-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] disabled:cursor-not-allowed disabled:opacity-60"
              >
                取消
              </button>
              <button
                ...
                onClick={onConfirm}
                disabled={isDeleting}
                className="inline-flex h-9 items-center gap-2 rounded-[7px] bg-destructive px-3 text-sm font-medium text-white transition hover:bg-destructive/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive/40 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {isDeleting ? <LoaderCircle className="size-4 animate-spin" /> : null}确认
              </button>
            </div>
          </Dialog.Popup>
        </Dialog.Viewport>
