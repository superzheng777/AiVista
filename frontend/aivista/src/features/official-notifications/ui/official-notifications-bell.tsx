...
import {
  ...
  interactionNotificationQueryKeys,
  listInteractionNotifications,
  markAllInteractionNotificationsRead,
  markInteractionNotificationRead,
} from "@/features/interaction-notifications/api/interaction-notifications-api";
import {
...
export function retainFirstNotificationPage(client: ReturnType<typeof useQueryClient>, queryKey: readonly string[]) {
  client.setQueryData<{ pages: unknown[]; pageParams: unknown[] }>(queryKey, (current) =>
    current ? { pages: current.pages.slice(0, 1), pageParams: current.pageParams.slice(0, 1) } : current,
  );
...
export function OfficialNotificationsBell() {
  ...
  const [openImageError, setOpenImageError] = useState<string | null>(null);
  const unread = useQuery({
    queryKey: officialNotificationQueryKeys.unreadCount,
    queryFn: fetchNotificationUnreadCount,
  });
  useEffect(() => {
    if (notificationRefreshVersion || publicationRefreshVersion)
      void client.invalidateQueries({ queryKey: officialNotificationQueryKeys.unreadCount });
    if (open && tab === "interaction" && notificationRefreshVersion)
      void retainFirstNotificationPage(client, interactionNotificationQueryKeys.list);
    if (open && tab === "official" && publicationRefreshVersion)
      void retainFirstNotificationPage(client, officialNotificationQueryKeys.list);
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => {
          setTab(unread.data?.officialUnreadCount ? "official" : "interaction");
          setOpen(true);
        }}
        aria-label="消息中心"
      ...
      >
        ...
        {unread.data?.totalUnreadCount ? (
          <span className="absolute -right-0.5 -top-0.5 grid min-w-4 place-items-center rounded-full bg-destructive px-1 text-[10px] leading-4 text-white">
            {unread.data.totalUnreadCount > 99 ? "99+" : unread.data.totalUnreadCount}
          </span>
        ) : null}
      </button>
      <Dialog.Root open={open} modal onOpenChange={setOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Viewport className="fixed inset-0 z-50">
            <Dialog.Popup className="absolute inset-y-0 left-0 flex w-full max-w-md flex-col border-r border-border bg-card shadow-2xl md:left-24">
              {image ? (
                <MessageImage image={image} onClose={() => setImage(null)} />
              ) : (
                <>
                  <header className="flex items-center justify-between border-b border-border px-5 py-4">
                    <div>
                      ...
                      <div className="mt-3 flex gap-1 rounded-lg bg-muted p-1">
                        <TabButton active={tab === "interaction"} onClick={() => setTab("interaction")}>
                          互动
                        </TabButton>
                  ...
                  {openImageError ? (
                    <p role="status" className="px-5 py-3 text-sm text-destructive">
                      {openImageError}
                    </p>
                  ) : null}
                  {tab === "interaction" ? (
                    <InteractionList open={open} openingImageId={openingImageId} onOpenImage={openImage} />
                  ) : (
                    <OfficialList open={open} openingImageId={openingImageId} onOpenImage={openImage} />
                  )}
                </>
              )}
            </Dialog.Popup>
      ...
      {publicDetail.openError ? (
        <PublicImageOpenError message={publicDetail.openError} onDismiss={publicDetail.dismissOpenError} />
      ) : null}
    </>
  );
}
...
function TabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-md px-3 py-1.5 text-xs font-medium ${active ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}
    >
      {children}
    </button>
  );
...
function InteractionList({
  ...
  onOpenImage: (image: GenerationAsset) => Promise<void>;
}) {
  return (
    <NotificationList<InteractionNotification>
      open={open}
      queryKey={interactionNotificationQueryKeys.list}
      fetchPage={listInteractionNotifications}
      markRead={markInteractionNotificationRead}
      markAllRead={markAllInteractionNotificationsRead}
      remove={deleteInteractionNotification}
      removeMany={deleteInteractionNotifications}
      renderItem={(item, read, deleting, managing, selected, toggle) => {
          ...
          <li
            ...
            className={
              item.readAt
                ...
                : "rounded-xl border border-[var(--accent-border)] bg-[var(--accent-soft)] p-3"
            }
          >
            <div className="flex gap-3">
              {managing ? (
                <input
                  aria-label={`选择消息 ${item.id}`}
                  type="checkbox"
                  checked={selected}
                  onChange={toggle}
                  className="mt-1"
              ...
              <button type="button" onClick={read} className="min-w-0 flex-1 text-left">
                <p className="text-sm">
                  <b>{item.actor.nickname}</b> {item.eventType === "USER_FOLLOWED" ? "关注了你" : "赞了你的作品"}
                </p>
              ...
              {image?.imageUrls.thumbnail ? (
                <button
                  type="button"
                  disabled={openingImageId === image.id}
                  onClick={() => {
                    read();
                    void onOpenImage(image);
                  }}
              ...
              {!managing ? <DeleteButton onDelete={deleting} /> : null}
            </div>
          </li>
        );
      }}
...
function OfficialList({
  ...
  return (
    <NotificationList<OfficialNotification>
      open={open}
      queryKey={officialNotificationQueryKeys.list}
      fetchPage={listOfficialNotifications}
      markRead={markOfficialNotificationRead}
      markAllRead={markAllOfficialNotificationsRead}
      remove={deleteOfficialNotification}
      removeMany={deleteOfficialNotifications}
      renderItem={(item, read, deleting, managing, selected, toggle) => {
        const failed = isPublicationRejected(item) || isPublicationFailed(item);
        const image = item.image;
        return (
          <li
            key={item.id}
          ...
          >
            <div className="flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                <button type="button" onClick={read} className="w-full text-left">
                  <p className={`text-sm font-medium ${failed ? "text-destructive" : "text-emerald-600"}`}>
                  ...
                  {item.violations.length ? (
                    <p className="mt-2 text-xs text-destructive">{violationText(item.violations)}</p>
                  ) : null}
                </button>
                {image ? (
                  <button
                    ...
                    className="mt-3 inline-flex items-center gap-1 text-xs disabled:opacity-60"
                  >
...
function NotificationList<T extends Message>({
  open,
  queryKey,
  fetchPage,
  markRead,
  ...
  const refresh = () => {
    void retainFirstNotificationPage(client, queryKey);
    void client.invalidateQueries({ queryKey: officialNotificationQueryKeys.unreadCount });
  };
  const read = useMutation({ mutationFn: markRead, onSuccess: refresh });
  const allRead = useMutation({ mutationFn: markAllRead, onSuccess: refresh });
  const removeOne = useMutation({ mutationFn: remove, onSuccess: refresh });
  const bulk = useMutation({
    mutationFn: removeMany,
    onSuccess: () => {
      setSelected(new Set());
      setManaging(false);
      refresh();
      ...
      <div className="flex-1 overflow-y-auto p-4">
          ...
          <ul className="space-y-2">
            {items.map((item) =>
              renderItem(
                ...
                () => {
                  if (window.confirm("删除这条消息？")) removeOne.mutate(item.id);
                },
                managing,
        ...
        {managing && selected.size ? (
          <button
            type="button"
            onClick={() => {
              if (window.confirm(`删除已选 ${selected.size} 条消息？`)) bulk.mutate([...selected]);
            }}
            className="mt-4 h-9 w-full rounded-lg bg-destructive text-sm text-white"
          >
            删除已选 {selected.size} 条
          </button>
        ) : null}
        {list.hasNextPage ? (
          <button
            ...
            className="mt-4 h-9 w-full rounded-lg border text-sm"
          >
            加载更多
          </button>
        ) : null}
...
function ListToolbar({
  ...
  return (
    <div className="flex items-center justify-between border-b px-5 py-3 text-xs">
      <span>{unread ? `${unread} 条未读` : "没有未读消息"}</span>
...
function Empty() {
  return (
    <div className="grid h-full place-items-center text-center text-sm text-muted-foreground">
      <div>
        <FolderOpen className="mx-auto size-6" />
        <p className="mt-2">暂无消息</p>
      </div>
    </div>
  );
}
function MessageImage({ image, onClose }: { image: GenerationAsset; onClose: () => void }) {
  return <OfficialAssetDetail image={image} onClose={onClose} />;
}
function OfficialAssetDetail({ image, onClose }: { image: GenerationAsset; onClose: () => void }) {
  ...
  const refreshImage = async (imageId: string) => {
    const refreshed = await getGenerationAsset(imageId);
    setCurrent(refreshed);
    return refreshed;
  };
  const favorite = useMutation({
    mutationFn: (value: boolean) => setGenerationImageFavorites([current.id], value),
    onSuccess: (_result, value) => setCurrent((asset) => ({ ...asset, favorited: value })),
  });
  const remove = useMutation({ mutationFn: () => deleteGenerationAssets([current.id]), onSuccess: onClose });
  return (
    <AssetDetail
      asset={current}
      ...
      publishDialog={
        publish ? (
          <PublicationFormDialog
            ...
            onSuccess={(result) => {
              setPublish(false);
              setCurrent((asset) => ({ ...asset, publicationReviewStatus: result.status }));
            }}
          />
        ) : null
      }
