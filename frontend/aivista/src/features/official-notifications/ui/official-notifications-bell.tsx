...
import { deleteOfficialNotification, deleteOfficialNotifications, fetchNotificationUnreadCount, listOfficialNotifications, markAllOfficialNotificationsRead, markOfficialNotificationRead, officialNotificationQueryKeys } from "@/features/official-notifications/api/official-notifications-api";
...
export function OfficialNotificationsBell() {
  ...
  const [open, setOpen] = useState(false); const [tab, setTab] = useState<Tab>("official"); const [image, setImage] = useState<GenerationAsset | null>(null); const [openingImageId, setOpeningImageId] = useState<string | null>(null); const [openImageError, setOpenImageError] = useState<string | null>(null);
  const unread = useQuery({ queryKey: officialNotificationQueryKeys.unreadCount, queryFn: fetchNotificationUnreadCount });
  useEffect(() => {
