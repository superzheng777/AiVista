...
type GenerationAssetDto = { imageId: string; url: string; urlExpiresAt: string; width: number; height: number; createdAt: string; finalPrompt: string; finalNegativePrompt: string | null };

export const assetQueryKeys = { all: ["assets"] as const, pages: () => [...assetQueryKeys.all, "pages"] as const };

function toAsset(dto: GenerationAssetDto): GenerationAsset { return { id: dto.imageId, url: dto.url, urlExpiresAt: dto.urlExpiresAt, width: dto.width, height: dto.height, createdAt: dto.createdAt, finalPrompt: dto.finalPrompt, finalNegativePrompt: dto.finalNegativePrompt }; }
