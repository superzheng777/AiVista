...
export function AssetsWorkspace() {
  ...
  async function refreshAsset(imageId: string): Promise<GenerationAsset> {
    ...
    queryClient.setQueryData<GenerationAsset[]>(assetQueryKeys.all, (current) => current?.map((item) => item.id === refreshed.id ? refreshed : item));
    setDetailAsset((asset) => asset?.id === refreshed.id ? refreshed : asset);
    return refreshed;
  }
...
function EmptyAssets() { return <section className="mt-16 rounded-2xl border border-dashed border-border bg-card px-6 py-16 text-center"><FolderOpen className="mx-auto size-7 text-muted-foreground" /><h2 className="mt-4 text-lg font-semibold">尚无生成图片</h2><p className="mt-2 text-sm text-muted-foreground">开始一次创作后，成功生成的图片会保存在这里。</p><Link href="/generate" className="mt-5 inline-flex h-9 items-center rounded-lg bg-primary px-3 text-sm font-medium text-primary-foreground">前往生成</Link></section>; }
