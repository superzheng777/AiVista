...
export function PublicInspirationCard({ image, priority = false, onOpen }: { image: GenerationAsset; priority?: boolean; onOpen?: (image: GenerationAsset) => void | Promise<void> }) {
  ...
  return <article ref={cardRef} className="group w-full"><WorkPreviewCardSurface><WorkPreviewCardImage image={image}><Link href={detailHref} prefetch={false} onClick={open} className="block size-full">{source ? <img src={source} alt={image.title ?? "公开作品"} loading={priority ? "eager" : "lazy"} decoding="async" referrerPolicy="no-referrer" onError={() => void refreshAfterError()} className="block size-full object-cover" /> : <span className="absolute inset-0 animate-pulse bg-[var(--skeleton)]" aria-hidden />}</Link>{source ? <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-[var(--primary)]/[0.28] opacity-0 transition-opacity group-hover:opacity-100" /> : null}<div aria-hidden="true" className="absolute right-3 top-3 z-10 flex gap-2 opacity-0 transition group-hover:opacity-100"><span className="grid size-9 place-items-center rounded-[6px] bg-[var(--primary)]/90 text-[var(--surface-bg)]"><Heart className="size-4" /></span><span className="grid size-9 place-items-center rounded-[6px] bg-[var(--primary)]/90 text-[var(--surface-bg)]"><MoreHorizontal className="size-5" /></span></div><Link href="/generate" className="absolute bottom-[14px] left-1/2 z-10 inline-flex h-[38px] -translate-x-1/2 items-center gap-2 whitespace-nowrap rounded-[7px] bg-[var(--primary)]/90 px-[18px] text-xs font-medium text-[var(--surface-bg)] opacity-0 transition group-hover:opacity-100"><Sparkles className="size-4" />以此为灵感</Link></WorkPreviewCardImage><WorkPreviewCardInfoBar title={image.title ?? "未命名作品"} trailing={<WorkPreviewCardLikes count={image.likeCount} />} /></WorkPreviewCardSurface></article>;
}
