...
type CreationFormDto = { formId: string; status: CreationForm["status"]; form: CreationForm["form"];
  answers: CreationForm["answers"]; requestedAt: string; resolvedAt: string | null };
type GenerationTurnDto = { creationId: string; mode: "NORMAL" | "AGENT"; status: "RUNNING" | "WAITING_INPUT" | "SUCCEEDED" | "FAILED" | "CANCELLED"; failureCode: string | null; revision: number; userMessage: ConversationMessageDto; assistantMessage: ConversationMessageDto | null; normalGenerationRequest: { negativePrompt: string | null } | null; generations: GenerationTaskDto[]; activities: CreationActivityDto[]; forms: CreationFormDto[] };
...
export type ResolveAgentFormInput = { creationId: string; formId: string; expectedRevision: number;
  action: "SUBMIT" | "SKIP"; answers: Record<string, AgentFormAnswer> | null };
...
export async function resolveAgentForm(input: ResolveAgentFormInput): Promise<ResolvedAgentForm> { const response = await browserApiClient.put<ApiResponse<{ creationId: string; revision: number; form: CreationFormDto }>>(`/agent-creations/${input.creationId}/forms/${input.formId}/response`, { expectedRevision: input.expectedRevision, action: input.action, answers: input.answers }); const data = unwrapApiResponse(response.data); return { creationId: data.creationId, revision: data.revision, form: toForm(data.form) }; }

function toForm(dto: CreationFormDto): CreationForm { return { id: dto.formId, status: dto.status,
  form: dto.form, answers: dto.answers, requestedAt: dto.requestedAt, resolvedAt: dto.resolvedAt }; }
