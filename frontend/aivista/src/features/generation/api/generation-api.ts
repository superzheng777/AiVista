...
type CreationActivityDto = { activityKey: string; sequenceNo: number; type: "NARRATION" | "SKILL" | "TOOL"; state: "RUNNING" | "COMPLETED" | "FAILED" | "CANCELLED"; content: string; toolName: string | null; generationTaskId: string | null; startedAt: string; completedAt: string | null };
...
type CreatedAgentCreationDto = { creationTaskId: string; sessionId: string; status: "RUNNING"; revision: number; createdAt: string };
...
export async function createAgentCreation(input: CreateAgentCreationInput, idempotencyKey: string): Promise<CreatedCreation> { const response = await browserApiClient.post<ApiResponse<CreatedAgentCreationDto>>("/agent-creations", input, { headers: { "Idempotency-Key": idempotencyKey } }); const data = unwrapApiResponse(response.data); return { id: data.creationTaskId, sessionId: data.sessionId }; }
...
export async function cancelAgentCreation(creationTaskId: string): Promise<void> { const response = await browserApiClient.post<ApiResponse<unknown>>(`/agent-creations/${creationTaskId}/cancel`); unwrapApiResponse(response.data); }
