...
type GenerationTaskDto = { taskId: string; sessionId: string; status: GenerationTaskStatus; taskVersion: number; retryCount: number; maxRetryCount: number; requestedImageCount: number; completedImageCount: number; failedImageCount: number; cancelledImageCount: number; failureCode: string | null; failureMessage: string | null; images: GenerationImageDto[]; createdAt: string; completedAt: string | null };
...
export async function listActiveGenerationTasks(): Promise<GenerationTask[]> { const response = await browserApiClient.get<ApiResponse<GenerationTaskDto[]>>("/generation-tasks/active"); return unwrapApiResponse(response.data).map(toTask); }
