...
type GenerationImageDto = { imageId: string; sourceIndex: number; url: string | null; urlExpiresAt: string | null; width: number; height: number };
type GenerationTaskDto = { taskId: string; sessionId: string; status: GenerationTaskStatus; taskVersion: number; requestedImageCount: number; completedImageCount: number; failedImageCount: number; cancelledImageCount: number; failureCode: string | null; failureMessage: string | null; images: GenerationImageDto[]; createdAt: string; completedAt: string | null };
...
export type CreateGenerationTaskInput = { sessionId?: string; prompt: string; negativePrompt?: string; aspectRatio: string; imageCount: number };
...
function toImage(dto: GenerationImageDto): GenerationImage { return { id: dto.imageId, sourceIndex: dto.sourceIndex, url: dto.url, urlExpiresAt: dto.urlExpiresAt, width: dto.width, height: dto.height }; }
function toTask(dto: GenerationTaskDto): GenerationTask {
  return { id: dto.taskId, sessionId: dto.sessionId, status: dto.status, version: dto.taskVersion, requestedImageCount: dto.requestedImageCount, completedImageCount: dto.completedImageCount, failedImageCount: dto.failedImageCount, cancelledImageCount: dto.cancelledImageCount, failureCode: dto.failureCode, failureMessage: dto.failureMessage, images: dto.images.map(toImage), createdAt: dto.createdAt, completedAt: dto.completedAt };
}
...
export async function getGenerationConsent(): Promise<GenerationConsent> { const response = await browserApiClient.get<ApiResponse<GenerationConsentDto>>("/users/me/consents/generation"); return toConsent(unwrapApiResponse(response.data)); }
export async function confirmGenerationConsent(policyVersion: string): Promise<GenerationConsent> { const response = await browserApiClient.post<ApiResponse<GenerationConsentDto>>("/users/me/consents/generation", { policyVersion }); return toConsent(unwrapApiResponse(response.data)); }
...
export async function updateGenerationSessionTitle(sessionId: string, title: string): Promise<GenerationSession> { const response = await browserApiClient.patch<ApiResponse<GenerationSessionDto>>(`/generation-sessions/${sessionId}`, { title }); return toSession(unwrapApiResponse(response.data)); }
