...
describe("conversation bottom following", () => {
  ...
  it("pauses after an upward scroll beyond the threshold", () => {
    expect(nextBottomFollowState(true, BOTTOM_FOLLOW_THRESHOLD_PX + 1, true)).toBe(false);
  });

  it("resumes when the user returns near the bottom", () => {
    expect(nextBottomFollowState(false, BOTTOM_FOLLOW_THRESHOLD_PX, false)).toBe(true);
  });
});
