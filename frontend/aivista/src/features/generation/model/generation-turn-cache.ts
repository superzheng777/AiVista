...
export function mergeGenerationTurnPages(
  ...
  return { ...incoming, pages: incoming.pages.map((page) => ({ ...page, items: page.items.map((turn) => {
    ...
    return existing && existing.revision > turn.revision
      ? { ...turn, status: existing.status, revision: existing.revision, forms: existing.forms,
          activities: existing.activities, assistantMessage: existing.assistantMessage, generations }
      : { ...turn, generations };
  }) })) };
}
...
export function applyAgentFormUpdateToTurns(
  ...
  return { ...current, pages: current.pages.map((page) => ({ ...page, items: page.items.map((turn) => {
    ...
    const forms = existing < 0 ? [...turn.forms, form] : turn.forms.map((item, index) => index === existing ? form : item);
