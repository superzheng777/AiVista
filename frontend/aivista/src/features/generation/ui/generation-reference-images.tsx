    ...
    <span className="group relative inline-flex size-7 shrink-0">
      ...
      {onRemove ? (
        <button
          ...
          onClick={onRemove}
          aria-label="移除参考图片"
          className="absolute -right-1.5 -top-1.5 inline-flex size-4 items-center justify-center rounded-full bg-[var(--primary)] text-[var(--surface-bg)] opacity-0 transition group-hover:opacity-100 focus-visible:opacity-100"
        >
          <X className="size-3" />
        </button>
      ) : null}
...
function AssetPickerDialog({
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="reference-asset-picker-title"
    ...
    >
      <section className="flex max-h-[min(42rem,calc(100dvh-2rem))] w-full max-w-3xl flex-col rounded-[10px] border border-[var(--border)] bg-[var(--surface-bg)] shadow-2xl">
        <header className="flex items-start justify-between gap-4 border-b border-[var(--border)] px-5 py-4">
          <div>
            <h2 id="reference-asset-picker-title" className="text-lg font-semibold">
              从资产添加
            </h2>
            <p className="mt-1 text-sm text-[var(--text-secondary)]">最多选择 3 张参考图片，顺序即提交给模型的顺序。</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            aria-label="关闭资产选择"
            className="inline-flex size-9 items-center justify-center rounded-[6px] hover:bg-[var(--surface-soft)]"
          >
            <X className="size-5" />
          </button>
        </header>
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          {assets.data?.length === 0 ? (
            <p className="py-12 text-center text-sm text-[var(--text-secondary)]">你还没有可用的图片资产。</p>
          ) : null}
          {assets.data?.length ? (
              ...
              {assets.data.map((asset) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSelected ? (
                      <span className="absolute right-1.5 top-1.5 inline-flex size-5 items-center justify-center rounded-full bg-[var(--accent)] text-xs font-bold text-white">
                        {draft.findIndex((image) => image.id === asset.id) + 1}
                      </span>
                    ) : null}
                  </button>
                );
        ...
        <footer className="flex items-center justify-between gap-3 border-t border-[var(--border)] px-5 py-4">
          <span className="text-sm text-[var(--text-secondary)]">
            已选择 {draft.length} / {MAX_REFERENCE_IMAGES}
          </span>
          <div className="flex gap-2">
            ...
            <button
              ...
              className="h-9 rounded-[6px] bg-[var(--primary)] px-4 text-sm font-medium text-[var(--surface-bg)]"
            >
              完成
            </button>
          </div>
...
export function GenerationReferenceImages({ value, disabled, onChange }: GenerationReferenceImagesProps) {
  ...
  return (
    <div className="flex min-h-7 flex-wrap items-center gap-1.5 px-6 pb-1 sm:px-[26px]" aria-label="已添加的参考图片">
      {value.map((asset, index) => (
        <ReferenceThumbnail
          key={asset.id}
          asset={asset}
          onRemove={disabled ? undefined : () => onChange(value.filter((_, currentIndex) => currentIndex !== index))}
        />
      ))}
    </div>
  );
...
export function GenerationReferenceImagePicker({
  value,
  disabled,
  isMenuOpen,
  isAuthenticated,
  ...
  onMenuOpenChange,
  renderTrigger,
}: GenerationReferenceImagePickerProps) {
      ...
      {value.length < MAX_REFERENCE_IMAGES ? (
        <span className="relative inline-flex">
          ...
          {isMenuOpen ? (
            <div
              role="menu"
              className="absolute bottom-[calc(100%+8px)] left-0 z-40 w-40 rounded-[9px] border border-[var(--border)] bg-[var(--surface-bg)] p-1.5 shadow-[0_8px_18px_rgb(43_35_25_/_14%)]"
            >
              <button
                type="button"
                role="menuitem"
                onClick={selectLocal}
                className="flex h-9 w-full items-center gap-2 rounded-[5px] px-2 text-left text-sm hover:bg-[var(--surface-soft)]"
              >
