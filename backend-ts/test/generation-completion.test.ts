...
describe("generation completion contract", () => {
  it("contains one final result without provider or transfer phase fields", () => {
    ...
    expect(completion).toEqual({ contractVersion: 1, generationTaskId: "42",
      expectedRevision: 3, outcome: "COMPLETED", providerRequestId: "provider-request", expectedImageCount: 2,
  ...
  it("uses a discriminated failure shape without success-only null fields", () => {
    ...
    expect(completion).toEqual({ contractVersion: 1, generationTaskId: "42",
      expectedRevision: 3, outcome: "FAILED", failureCode: "IMAGE_TRANSFER_FAILED", providerRequestId: null });
