...
export class GenerationCompletionClientService {
  ...
  async reportPhase(taskId: bigint, phase: "GENERATING" | "SAVING",
    ...
    if (!this.token) throw new Error("Generation worker token is not configured");
    const response = await putJavaWorker({
      url: `${this.baseUrl}/internal/generation-worker/tasks/${taskId}/phase`,
      token: this.token,
      body: { phase },
      timeoutMs: this.timeoutMs,
      signal,
      fallbackError: "Java generation phase API request failed",
    });
  ...
  private async putCompletion(completion: GenerationCompletion): Promise<GenerationCompletionResponse> {
    ...
    const response = await putJavaWorker({ url, token: this.token, body: completion,
      timeoutMs: this.timeoutMs, signal: undefined,
      fallbackError: "Java generation worker API request failed" });
    return responseSchema.parse(await response.json());
  }
}
