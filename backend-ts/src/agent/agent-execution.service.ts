...
export class AgentExecutionService {
  ...
  async execute(command: AgentExecuteMessage, signal?: AbortSignal): Promise<boolean> {
    ...
    this.active.set(id, { revision: command.expectedRevision, abort: cancellation });
    try {
      ...
      try {
        ...
        const inputImages = await this.images.load(snapshot.inputAssets, runSignal);
        ...
        const tools = [createSkillReadTool(cwd), createInspectImageTool({ inspect: async (assetId, toolSignal) => {
        ...
        const result = await runAgentPrompt({ binding, prompt: snapshot.prompt, context: snapshot.agentContext,
